...
void AFortPlayerPawn::ServerHandlePickupHook(AFortPlayerPawn* Pawn, AFortPickup* Pickup, float InFlyTime, FVector InStartDirection, bool bPlayPickupSound)
{
	...
	//added by Boosted
	FVector pLoc = Pawn->GetActorLocation();
	FVector iLoc = Pickup->GetActorLocation();

	FVector difference = iLoc - pLoc;
	...
	if (distance >= 600)
	{
		Pawn->GetController()->ClientReturnToMainMenu(L"An error occured.\nIf this keeps happening please open a ticket in our discord server.\n\ndiscord.gg/exitfn\nError Code: AC_SHPH");
		return;
	}

	if (Globals::bDisableLootingOnSpawnIsland && !Globals::bStarted)
		return;
